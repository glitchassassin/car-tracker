...
- As a handoff volunteer, I want to see which cars are ready for pickup so I can
  prepare
- As a handoff volunteer, I want to quickly mark a car as "Done" when I receive
  the keys
...
- **Internet connectivity issues:** Users must maintain connection for live
  updates; clear indicators when connection is lost with instructions to
  reconnect
...
- Build all three volunteer interfaces, with loaders and actions to read and
  write from db
...
- Disconnect recovery (refreshing page when Internet connection is
  reestablished)
